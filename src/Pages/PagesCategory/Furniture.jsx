...
const Forniture = () => {
  ...
  const [furnitureAuctions, setFurnitureAuctions] = useState([]);

  useEffect(() => {
    fetch("http://127.0.0.1:8000/api/categories/show/all/")
      .then((response) => response.json())
      .then((data) => setFurnitureAuctions(data))
...
//-----------Esto sirve solo para generar productos aleatorios-------------
  ...
  for (let i = 1; i <= 10; i++) {
    ...
    furnitureAuctions.push(producto);
  }
  console.log(furnitureAuctions);
    ...
    <div>
      ...
      <div className="container mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {furnitureAuctions.map((producto) => (
            <AuctionItem
              key={producto.id}
